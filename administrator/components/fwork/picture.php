...
class picture {
    ...
    public $image;
    public $image_type;
    private $image_typeNew;
    public $image_width;
    public $image_height;
   
   
    public function __construct($image_file,$ext) {
        ...
        switch($ext) {
            case "gif": $this->image_type = 'gif'; break;
            case "jpeg": $this->image_type = 'jpeg'; break;
            case "jpg": $this->image_type = 'jpeg'; break;
            case "png": $this->image_type = 'png'; break;
            default: $this->image_type = ''; break;
        }

        $this->fotoimage();
    }
     public function typerimage() {
		 return $this->image_typeNew;
	 }
    ...
    public function imagesave($convert_toj=true, $image_file=NULL, $image_compress=100,$image_name) {
        if($image_file==NULL) {
            switch($this->image_type) {
        ...
        if($convert_toj==true){
            $this->image_typeNew="jpeg";
                imagejpeg($this->image, $image_file.$image_name.'.jpeg', $image_compress);
        }else{
            switch($this->image_type) {
                case 'gif': imagegif($this->image, $image_file); break;
                case 'jpeg': imagejpeg($this->image, $image_file.$image_name.'.jpeg', $image_compress); break;
