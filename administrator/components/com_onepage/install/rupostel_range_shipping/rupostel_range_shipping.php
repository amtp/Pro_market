...
/* RuposTel Range Shipping -> based on VM3 API and weight countries triggers */
...
class plgVmShipmentRupostel_range_shipping extends vmPSPlugin {
	...
	function getTableSQLFields () {
		...
		$SQLfields = array(
			...
			'order_total'				   => 'decimal(10,4)',
			'order_currency'			   => 'int(11)',
    ...
    private function _getRangePrice($total, $method)
	{
		foreach ($method->ranges as $range)
		{
			...
			if ((empty($from)) && (empty($to)) && (empty($price))) continue; 
			
			if (($total >= $from) && (($total < $to) || (empty($to))))
			{
				$method->shipment_cost = $price; 
				return $price; 
			}
		}
	...
	function getCosts (VirtueMartCart $cart, $method, $cart_prices) {
		...
		$total = (float)$cart_prices['salesPrice']; 
		$this->_getRangePrice($total, $method); 
	...
	protected function checkConditions ($cart, $method, $cart_prices) {
		...
		if($allconditions === 5){
			...
			if (!empty($method->currency_filter))
			{
				$method->currency_filter = (int)$method->currency_filter; 
				$currency_id = (int)$cart->pricesCurrency; 
				if ($method->currency_filter != $currency_id) return false; 
				
			}
			
			if(isset($cart_prices['salesPrice'])){
			  $total = 	(float)$cart_prices['salesPrice']; 
			  $res = $this->_getRangePrice($total, $method); 
