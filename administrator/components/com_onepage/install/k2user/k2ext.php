<?php
/**
 * @version		$Id: k2.php 1897 2013-02-08 17:13:00Z lefteris.kavadas $
 * @package		K2
...
class plgUserK2 extends JPlugin
{
    ...
    function onAfterStoreUser($user, $isnew, $success, $msg)
    {
		...
		$isK2 = JRequest::getInt('K2UserForm', false); 
        if ($mainframe->isSite() && $task != 'activate' )
        {
            ...
            require_once (JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_k2'.DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'class.upload.php');
            $savepath = JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'k2'.DIRECTORY_SEPARATOR.'users'.DIRECTORY_SEPARATOR;
		    if ($isK2)
            ...
            if (JRequest::getBool('del_image'))
            {
                ...
                if (JFile::exists(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'k2'.DIRECTORY_SEPARATOR.'users'.DIRECTORY_SEPARATOR.$row->image))
                {
                    JFile::delete(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'k2'.DIRECTORY_SEPARATOR.'users'.DIRECTORY_SEPARATOR.$row->image);
                }
                $row->image = '';
            }
    ...
    function onBeforeStoreUser($user, $isNew)
    {
        ...
        if ($params->get('K2UserProfile') && $isNew && $params->get('recaptchaOnRegistration') && $mainframe->isSite() && !$session->get('socialConnectData'))
        {
            ...
            {
                require_once (JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_k2'.DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'recaptchalib.php');
            }
