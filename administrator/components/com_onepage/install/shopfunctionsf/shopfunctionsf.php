 ...
 * @version $Id: shopfunctionsf.php 9203 2016-04-06 18:25:56Z Milbo $
 */
...
class shopFunctionsF {
	...
	static public function getLastVisitedItemId ($default = 0) {
		if (self::isLess('30910')) return self::getLastVisitedItemId309($default); 
		$session = JFactory::getSession();
		return $session->get( 'vmlastvisitedItemid', $default, 'vm' );
	}

	static public function setLastVisitedItemId ($id) {
		if (self::isLess('30910')) return self::setLastVisitedItemId309($id); 
	...
	static public function getAddToCartButton ($orderable) {
		if (self::isLess('3094')) return self::getAddToCartButton309($orderable); 
		
		return self::renderVmSubLayout('addtocartbtn',array('orderable'=>$orderable));
	}
     public static function isFEmanager3090($task = FALSE) {
    ...
    public static function isFEmanager3094($task = FALSE) {
	    if (self::isLess('3090')) return self::isFEmanager3090($task); 
	...
	static public function isFEmanager ($task = 0) {
		if (self::isLess('3094')) return self::isFEmanager3094($task); 
	...
	static public function renderCountryList ($countryId = 0, $multiple = FALSE, $_attrib = array(), $_prefix = '', $required = 0, $idTag = 'virtuemart_country_id') {
		...
		if (self::isLess('3012')) 
			return self::renderCountryList3012($countryId, $multiple, $_attrib, $_prefix,$required);
		elseif (self::isLess('3014'))
			return self::renderCountryList3014($countryId, $multiple, $_attrib, $_prefix,$required,$idTag);
	...
	static public function getLastVisitedItemId309 ($default = 0) {
		$session = JFactory::getSession();
		return $session->get( 'vmlastvisItemid', $default, 'vm' );
	}

	static public function setLastVisitedItemId309 ($id) {
		$session = JFactory::getSession();
		return $session->set( 'vmlastvisItemid', (int)$id, 'vm' );
	}
	
	
	static public function getAddToCartButton309 ($orderable) {
	 ...
	 * @return string HTML containing the <select />
	 */
	static public function renderCountryList3012 ($countryId = 0, $multiple = FALSE, $_attrib = array(), $_prefix = '', $required = 0) {
	 ...
	 * @return string HTML containing the <select />
	 */
	static public function renderStateList3012 ($stateId = '0', $_prefix = '', $multiple = FALSE, $required = 0,$attribs=array()) {
	 ...
	 * @return string HTML containing the <select />
	 */
	static public function renderStateList3014 ($stateId = '0', $_prefix = '', $multiple = FALSE, $required = 0,$attribs=array(),$idTag = 'virtuemart_state_id') {
	 ...
	 * @return string HTML containing the <select />
	 */
	static public function renderCountryList3014 ($countryId = 0, $multiple = FALSE, $_attrib = array(), $_prefix = '', $required = 0, $idTag = 'virtuemart_country_id') {
	...
	static public function renderStateList ($stateId = '0', $_prefix = '', $multiple = FALSE, $required = 0,$attribs=array(),$idTag = 'virtuemart_state_id', $suffix='_field') {
		...
		if (self::isLess('3012')) 
			return self::renderStateList3012($stateId, $_prefix, $multiple, $required,$attribs);
		elseif (self::isLess('3014'))
			return self::renderStateList3014($stateId, $_prefix, $multiple, $required,$attribs,$idTag);
	...
	static public function renderMail ($viewName, $recipient, $vars = array(), $controllerName = NULL, $noVendorMail = FALSE,$useDefault=true) {
		...
		/* stAn addition so we can alter the VM emails completely */
		$dispatcher = JDispatcher::getInstance();
		JPluginHelper::importPlugin ('vmextended');
		$results = $dispatcher->trigger ('plgVmOnRenderEmail', array(&$viewName, &$recipient, &$vars, &$controllerName, &$noVendorMail, &$useDefault));
		...
		//double check vendor email: 
		if (empty($view->vendorEmail)) {
		   $config = JFactory::getConfig(); 
		   $view->vendorEmail = $config->get( 'mailfrom' );
		...
		if(!empty($vmempro_trigger) && ((isset($vars['newOrderData']['customer_notified']) && $vars['newOrderData']['customer_notified']==1)))
		{
			...
			foreach ($vmempro_trigger as $trigger){
				...
				if($res[0] === false){
				 ...
				 // at least one vmee plugin is enabled...
				 $isVMEE = true; 
				}
	...
	private static function sendVmMail (&$view, $recipient, $noVendorMail = FALSE) {
		...
		$dispatcher = JDispatcher::getInstance();
		JPluginHelper::importPlugin ('vmextended');
		$results = $dispatcher->trigger ('plgVmOnSendEmail', array(&$view, &$recipient, &$noVendorMail));
