...
class vm_ps_product extends vmAbstractObject {
	...
	function get_field( $product_id, $field_name, $force = false ) {
		...
		if( !isset($GLOBALS['product_info'][$product_id][$field_name] ) || $force ) {
			$q = 'SELECT p.product_id, a.category_name format, p.* FROM jos_vm_product p '.
                                'left outer join jos_vm_product_category_xref c on c.product_id=p.product_id '.
                                'left outer join jos_vm_category_xref b on b.category_child_id=c.category_id '.
                                'left outer join jos_vm_category a on a.category_id=b.category_parent_id '.
		...
		$retVal = $GLOBALS['product_info'][$product_id][$field_name];
                
                if ($field_name == "format" && trim($retVal) == "Musik") {
                    $retVal = "CD";
                }
		...
		|| $check_multiple_prices) {
			...
			elseif( !$price ) {
				...
				$price = $this->getPriceByShopperGroup( $product_id, $GLOBALS['vendor_info'][$vendor_id]['default_shopper_group_id'], $check_multiple_prices, $volume_quantity_sql );
                        ...
                        require_once(CLASSPATH . 'oblksnurra.php' );
                        $price['product_price'] = oblksnurra::getPris($product_id, $auth['user_id'], $price['product_price']);
                        $GLOBALS['product_info'][$product_id]['price'] = $price; 
			return $price;
		}
		else {
