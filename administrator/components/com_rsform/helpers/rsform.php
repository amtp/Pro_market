...
class RSFormProHelper
{
    ...
    public static function displayForm($formId, $is_module = false)
    {
        ...
        if ($formparams && !empty($formparams->formProcessed)) {
            ...
            if ($formparams->showSystemMessage) {
                $mainframe->enqueueMessage(JText::_('Данные успешно отправлены. Спасибо!'));
            }
    ...
    public static function sendSubmissionEmails($SubmissionId, $only_return_replacements = false, $skip_globals = false)
    {
        ...
        if (!$skip_globals) {
            array_push($placeholders, '{global:username}', '{global:usercompany}', '{global:userid}', '{global:useremail}', '{global:fullname}', '{global:userip}', '{global:date_added}', '{global:sitename}', '{global:siteurl}', '{global:confirmation}', '{global:deletion}', '{global:submissionid}', '{global:submission_id}', '{global:mailfrom}', '{global:fromname}', '{global:formid}');
            array_push($values, $user->username, $user->company, $user->id, $user->email, $user->name, $submission->UserIp, RSFormProHelper::getDate($submission->DateSubmitted), $config->get('sitename'), JUri::root(), $confirmation, $deletion, $submission->SubmissionId, $submission->SubmissionId, $config->get('mailfrom'), $config->get('fromname'), $formId);
        }
    ...
    public static function showForm($formId, $val = array(), $validation = array())
    {
        ...
        $global = array(
            ...
            '{global:usercompany}' => $user->get('company'),
        ...
        if (empty($invalid)) {
            ...
            foreach ($formComponents as $component) {
                ...
                if (file_exists($fieldTypeFile)) {
                    ...
                    $field->processBeforeStore($SubmissionId, $post, $files, $form->ScriptProcessFile);
                }
            }
            ...
            if ($form->ShowContinue) {
                ...
                // Continue button
                $continueButtonLabel = JText::_('Продолжить');
                $continueButton = '<input type="button" class="rsform-submit-button btn btn-primary" name="continue" value="' . JText::_('Продолжить') . '" onclick="' . $goto . '"/>';
                if (strpos($continueButtonLabel, 'input')) {
                    $continueButton = JText::sprintf('Продолжить', $goto);
                } else {
                    $layoutName = $form->FormLayoutName;
                    if (file_exists(dirname(__FILE__) . '/formlayouts/' . $layoutName . '.php')) {
                        require_once dirname(__FILE__) . '/formlayouts/' . $layoutName . '.php';
    ...
    public static function getForm($formId)
    {
        ...
        if (!isset($form[$formId])) {
            ...
            $query->clear()
                ...
                ->select($db->qn('ScriptProcessFile'))
