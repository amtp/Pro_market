...
class CategoriesModelCategories extends JModelList
{
	...
	protected function populateState($ordering = 'a.lft', $direction = 'asc')
	{
		...
		$this->setState('parnt_id', $this->getUserStateFromRequest($this->context . '.parnt_id', 'cat_id', '0', 'string'));
	 ...
	 * @since   1.6
	 */
	protected function getListQuery($cat_id='0')
	{
	    //8 организации
		...
		$listarrprnt= [];
        $cat_id= $this->getState('parnt_id');
        if ($cat_id != '' && $cat_id!='0')
        {
            $queryP = $db->getQuery(true);
            $queryP->select('a.id');
            $queryP->from('#__categories AS a');
            $queryP->where('a.parent_id = ' . (int)$cat_id);
            $db->setQuery($queryP);
//loadObjectList   loadRowList
        ...
        // $query->where('a.id = '.(int)$cat_id);

            $query->where('(a.parent_id = '.(int)$cat_id.' OR a.parent_id IN ('.$listarrp_rnt.'))');
       // echo $listarrp_rnt;
