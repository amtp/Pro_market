...
class SiteMenu extends AbstractMenu
{
	...
	public function load()
	{
		...
		$loader = function () use ($db)
		{
            ...
            switch ($ismobil)
            {
                case 0:$ismobil=1;break;
                case 1:$ismobil=2;break;
            }
			$query = $db->getQuery(true)
				->select('m.id, m.menutype, m.title, m.alias, m.note, m.path AS route, m.link, m.type, m.level, m.language, m.is_mobile')
				...
				->where('m.is_mobile in (0,'.$ismobil.')')
				->order('m.lft');
