...
abstract class Factory
{
    ...
    public static $configc = null;
    public static $configt = null;
    ...
    public static $databaseC = null;
    public static $databaseT = null;
    public static $ismobile = 0;
    ...
    public static function getGalleryList($fieldid, $uid = 0)
    {
        // return "f";
        $usr = self::getUser();
        $galleryhtml = array();
        if ($usr != null) {
            ...
            $query->from($db->quoteName("object_file"));
            $query->where($db->quoteName("object") . ' =' . $db->quote("object_company_images"));
            $query->where($db->quoteName("object_guid") . ' =' . $db->quote($usr->company));
            $db->setQuery($query);
    ...
    public static function getDboC()
    {
        if (!self::$databaseC) {
            self::$databaseC = self::createDboC();
        }

        return self::$databaseC;
    }

    /**
     * Create a database object
     *
     * @return  \СDatabaseDriver
     *
     * @see     \СDatabaseDriver
     * @since   1.7.0
     */
    protected static function createDboC()
    {
        $conf = self::getConfigC();
        ...
        try {
            ...
            jexit('Company Database Error: ' . $e->getMessage());
        }
    ...
    /**
     * Get a configuration object Company
     *
     * Returns the global {@link \CConfig} object, only creating it if it doesn't already exist.
     *
     * @param string $file The path to the configuration file
     * @param string $type The type of the configuration file
     * @param string $namespace The namespace of the configuration file
     ...
     * @since   1.7.0
     */
    public static function getConfigC($file = null, $type = 'PHP', $namespace = 'C')
    {
        // if (!self::$configc)
        {
            if ($file === null) {
                $file = JPATH_CONFIGURATION . '/copmanyconfig.php';
            }

            self::$configc = self::createConfig($file, $type, $namespace);
    ...
    public static function getCompanylogo()
    {
        ...
        if ($usr != null) {
            ...
            $query->select($db->quoteName("filename"));
            $query->from($db->quoteName("object_file"));
            $query->where($db->quoteName("object") . ' =' . $db->quote("common\models\Company"));
            $query->where($db->quoteName("object_guid") . ' =' . $db->quote($usr->company));
            $db->setQuery($query);
            $datas = $db->loadRowList();
            return $datas;
    ...
    public static function getFirmPrice()
    {
        // return "f";
        $usr = self::getUser();
        $galleryhtml = array();
        if ($usr != null) {
            ...
            $query->select($db->quoteName("filename"));
            $query->from($db->quoteName("object_file"));
            $query->where($db->quoteName("object") . ' =' . $db->quote("Company\price"));
            $query->where($db->quoteName("object_guid") . ' =' . $db->quote($usr->company));
            $db->setQuery($query);
            $datas = $db->loadResult();
            return $datas;
    ...
    public static function getDboT()
    {
        if (!self::$databaseT) {
            self::$databaseT = self::createDboT();
        }

        return self::$databaseT;
    }

    protected static function createDboT()
    {
        $conf = self::getConfigT();
    ...
    public static function getConfigT($file = null, $type = 'PHP', $namespace = 'C')
    {
        // if (!self::$configc)
        {
            if ($file === null) {
                $file = JPATH_CONFIGURATION . '/timeconfig.php';
            }

            self::$configt = self::createConfig($file, $type, $namespace);
