...
class CategoryView extends HtmlView
{
    ...
    protected function prepareDocument()
    {
        ...
        $dpdsz = "в ";
        if ($cityname == 'Владивосток') $dpdsz = "во ";

        if ($cityname == 'Все города') {
            $dpdsz = "";
            $rname_city = 'город не выбран';
        } else {
            $rname_city = \Joomla\CMS\Mdata\MdataHelper::to_prepositional($cityname);
        }
        ...
        $actividpr = $menu->getActive();

        if(isset($actividpr->parent_id)){
            if ($actividpr->parent_id != 1) {
                //  die($actividpr->parent_id);
                $actividpr = $menu->getItem($actividpr->parent_id);
                $parentidmenuid = $actividpr->id;
                while (isset($actividpr->parent_id)) {
                    $parentidmenuid = $actividpr->id;
                    $actividpr = $menu->getItem($actividpr->parent_id);
                }
                if ($parentidmenuid == 121) $title = 'Организации: раздел - ' . $title;
                if ($parentidmenuid == 533) $title = 'Объявления: раздел - ' . $title;
                if ($parentidmenuid == 265) $title = 'Недвижимость: раздел - ' . $title;
                if ($parentidmenuid == 264) $title = 'Работа: раздел - ' . $title;
            } else {

                if ($actividpr->id == 121) $title = $title.': Организации';
                if ($actividpr->id == 533) $title = $title . ' - ';
            }
        }
