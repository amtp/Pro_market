	   	...
	   	foreach( $rowFields['fields'] as $field) {
			...
			if( $field['type'] === 'delimiter')
			{
	   		...
	   		if( $field['name'] == 'agreed') {
				...
				document.write(\'<label for="agreed_field">'. str_replace("'","\\'",OPCLang::_('COM_VIRTUEMART_I_AGREE_TO_TOS')) .'</label><a href="javascript:void window.open(\\\''. JURI::base(true) .'/index2.php?option=com_virtuemart&page=shop.tos&pop=1\\\', \\\'win2\\\', \\\'status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no\\\');">\');
				document.write(\' ('.OPCLang::_('COM_VIRTUEMART_STORE_FORM_TOS') .')</a>\');
				//]]></script>
				<noscript>
					<label for="agreed_field">'. OPCLang::_('COM_VIRTUEMART_I_AGREE_TO_TOS') .'</label>
					<a target="_blank" href="'. JURI::base(true) .'/index.php?option=com_virtuemart&amp;page=shop.tos" title="'. OPCLang::_('COM_VIRTUEMART_I_AGREE_TO_TOS') .'">
					 ('.OPCLang::_('COM_VIRTUEMART_STORE_FORM_TOS').')
					</a></noscript>';
	   		}
	   		if( $field['name'] == 'username' && VM_REGISTRATION_TYPE == 'OPTIONAL_REGISTRATION' ) {
				echo '<div class="formLabel">
						<input type="checkbox" id="register_account" name="register_account" value="1" class="inputbox" onchange="showFields( this.checked, new Array(\'username\', \'password\', \'password2\') );" ';
