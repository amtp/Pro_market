...
class ContentViewArticle extends JViewLegacy
{
    ...
    protected function _prepareDocument()
    {
        ...
        if (isset($category->title)) $title = $title . ' - ' . $category->title;

        if (isset($this->item->Official_Name) && $this->item->Official_Name != "") $title = $this->item->Official_Name . ' - ' . $menu->title;

        $dpdsz = "";
        if (isset($this->item->cityname) && $this->item->cityname != "")
        {
            $dpdsz = "в";
            ...
            $dpdsz = ' ' . $dpdsz . ' ' . $rname_city;
        }


        if (substr($title, 0, 20) == 'Справочная') {
            $this->document->setTitle($title );
        } else {

            $this->document->setTitle($title .  $dpdsz );
        }
