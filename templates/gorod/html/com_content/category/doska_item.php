...
<?php if($count_photo > 100000) { ?>
	<div class="mod_news_img">
		<a href="<?php echo $link?>#link" title="<?php echo $this->item->title ?>"><span class="podlozhka"></span></a>
		<?php echo JHtml::_('content.prepare', '{gallery preview-width=255 preview-height=150}doska/'.$item_id.'{/gallery}'); ?>
	</div>
<?php } else { ?>
...
<div class="kat_item_info">
		...
		<div class="mini_icon">
			...
			<div class="ic_cat">
				...
				<span itemprop="streetAddress"><?php echo "г.".$this->item->cityname." ".$f[80] ?></span>
			</div>
			<?php } ?>
			<?php if($f[79]) { ?>
			<div class="price">
				<span><?php echo number_format($f[79],0,'',' ') ?></span> <?php echo $valuta1 ?> <?php if($f[82] == 'Да') { ?> <sup> возможен торг</sup><?php } ?>
			</div>
			<?php } ?>			
		</div
