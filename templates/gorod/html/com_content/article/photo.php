...
<?php } ?>
	...
	<div class="news_logo">
		<?php echo JHtml::_('content.prepare', '{gallery preview-width=450 preview-height=600}photo/uchastniki/'.$item_id.'{/gallery}'); ?>
	</div>
	</div>
	...
	<?php } ?>
		<?php if($ya_knopki == '1') { ?>
			<div class="social_button no_link">
				...
				<div class="ya-share2" data-services="vkontakte,facebook,odnoklassniki,moimir" data-counter="" data-image="<?php echo $domen;?><?php echo $images->image_intro;?>" data-description=''></div>	
			</div>
		<?php } ?>		
...
<div class="gallery">	
			<?php $dir = opendir(''.JPATH_BASE.'/images/photo/uchastniki/'.$item_id.'/gallary/');
				$count_gallery = 0;
				while($file = readdir($dir)){
				if($file == '.' || $file == '..' || is_dir(''.JPATH_BASE.'/images/photo/uchastniki/'.$item_id.'/gallary/' . $file)){
					continue;
				}
				$count_gallery++;
			} ?>
<?php if($count_gallery > 0) { ?>
	<?php echo JHtml::_('content.prepare', '{gallery preview-width=400 preview-height=350}photo/uchastniki/'.$item_id.'/gallary{/gallery}'); ?>
<?php } ?>
</div>
